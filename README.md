# FrostOptima v1.0

Retail profit optimization tool that compares margin-focused vs. volume-focused product strategies.

**[View Live App]((https://frostoptima.vercel.app/))** | **[GitHub Repository](https://github.com/cochisestarks-web/retail-profit-optimizer)**

## The Problem

During 20+ years in grocery retail operations, I observed a common industry challenge: product placement decisions often rely on intuition and margin percentages rather than data-driven analysis of total profit potential.

High-margin items naturally seem like the best choice for premium placement, but without calculating total weekly profit (margin × velocity), stores may miss optimization opportunities.

This tool demonstrates how data analysis can validate or challenge conventional assumptions about product prioritization strategies.

## Features

- **Universal Product Analysis**: Works for any product category (frozen, produce, electronics, etc.)
- **Strategy Comparison**: Compare HIGH MARGIN vs. HIGH VOLUME approaches side-by-side
- **Data-Driven Recommendations**: Shows which strategy generates more total profit
- **Flexible Input**: Analyze any products with cost, sell price, and demand data

## Demo

![FrostOptima Strategy Comparison](retailmarginprofitcalculator.jpg)

*Live calculator showing profit analysis and strategy comparison for frozen food products.*

## Technical Implementation

Built with Google AI Studio Code Assistant using:
- HTML/CSS/JavaScript
- Real-time calculation engine
- Responsive design
- Based on 20+ years retail operations experience

## Example Use Case

Demonstrated with Publix frozen food endcap (3-door ONRZH unit), but applicable to any retail product category or display configuration.

## Background

During 20+ years in grocery retail operations, I observed a common industry challenge: product placement decisions often rely on intuition and margin percentages rather than data-driven analysis of total profit potential. 

High-margin items naturally seem like the best choice for premium placement, but without calculating total weekly profit (margin × velocity), stores may miss optimization opportunities.

This tool demonstrates how data analysis can validate or challenge conventional assumptions about product prioritization strategies.

## Future Enhancements (v2.0)

- Visual display configurations (freezer, shelf, endcap)
- Demand forecasting integration
- Space constraint modeling
- Category-specific optimization rules
- Multi-week simulation

---

**Built by Derek Loa**  
Portfolio project demonstrating retail operations expertise + technical problem-solving
